...
- **Phase 6 (port real tool widgets into the workspace)** — IN PROGRESS, started 2026-05-03. New package `@lunedoc/tools` exists alongside `@lunedoc/ui` and `@lunedoc/i18n`. First widget shipped: **Merge PDF** at route `/merge-pdf` in `apps/web` (commit `450fad8`). `apps/web` now uses `react-router-dom` v7 with a shared `<Header>` / `<Footer>` layout and lang state shared across routes — switching locale on the Merge page updates both Header and Merge content simultaneously. The remaining 7 widgets (Split, Watermark, Sign, OCR, Edit, Compress, Convert) still live only in the prototype. See `docs/phase-6-tool-widget-port-plan.md`.
- **Next workstream** — Phase 6 Step 2 (port Split), or branch out to extract `I18N_ARTICLES`. See `docs/phase-6-tool-widget-port-plan.md` §3 for the full port order.
