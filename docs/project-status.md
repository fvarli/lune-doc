...
- **Phase 6 (port real tool widgets into the workspace)** — IN PROGRESS, started 2026-05-03. New package `@lunedoc/tools` alongside `@lunedoc/ui` and `@lunedoc/i18n`. **Six widgets ported so far** (6 of 8): Merge at `/merge-pdf` (`450fad8`), Split at `/split-pdf` (`876aee7`), Watermark at `/watermark-pdf` (`e90e54f` refactor + `7956f69` feat), Sign at `/sign-pdf` (`828a342`), OCR at `/ocr-pdf` (`4c2ce68`), Edit at `/edit-pdf` (`d5e71d1`). `apps/web` uses `react-router-dom` v7 with a shared `<Header>` / `<Footer>` layout and lang state shared across routes — locale switches on any route update Header, tool content, and Footer simultaneously. The remaining 2 widgets (**Compress, Convert**) still live only in the prototype. See `docs/phase-6-tool-widget-port-plan.md`.
- **Next workstream** — Phase 6 Step 7 (port **Compress** — unique because it's a state machine: empty → uploading → done). Then Step 8 (Convert — last widget; format-pair picker). After both ship, Phase 6 closes and we open Phase 7 (production marketing site / SEO pages, per `docs/seo-tool-page-template.md`) or backend MVP per `docs/backend-api-plan.md`.
