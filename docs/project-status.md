...
- **Phase 6 (port real tool widgets into the workspace)** — IN PROGRESS, started 2026-05-03. New package `@lunedoc/tools` alongside `@lunedoc/ui` and `@lunedoc/i18n`. **Four widgets ported so far:** Merge at `/merge-pdf` (`450fad8`), Split at `/split-pdf` (`876aee7`), Watermark at `/watermark-pdf` (`e90e54f` refactor + `7956f69` feat), Sign at `/sign-pdf` (`828a342`). `apps/web` uses `react-router-dom` v7 with a shared `<Header>` / `<Footer>` layout and lang state shared across routes — switching locale on any tool route updates Header, tool content, and Footer simultaneously. Sign exercises 3 nested mode switches (method × style × field type) and a callback-prop preview pattern. The remaining 4 widgets (OCR, Edit, Compress, Convert) still live only in the prototype. See `docs/phase-6-tool-widget-port-plan.md`.
- **Next workstream** — Phase 6 Step 5 (port OCR — first widget with a 2-pane preview interior: left scanned page + right recognized text). Or branch out to extract `I18N_ARTICLES`. See `docs/phase-6-tool-widget-port-plan.md` §3 for the full port order.
