...
- **Phase 6 (port real tool widgets into the workspace)** — IN PROGRESS, started 2026-05-03. New package `@lunedoc/tools` alongside `@lunedoc/ui` and `@lunedoc/i18n`. **Three widgets ported so far:** Merge PDF at `/merge-pdf` (`450fad8`), Split PDF at `/split-pdf` (`876aee7`), and Watermark PDF at `/watermark-pdf` (`e90e54f` refactor + `7956f69` feat). `apps/web` uses `react-router-dom` v7 with a shared `<Header>` / `<Footer>` layout and lang state shared across routes — switching locale on any tool route updates Header, tool content, and Footer simultaneously. Watermark also re-seeds its text input default per locale (`CONFIDENTIAL` / `GİZLİ` / `CONFIDENCIAL`). The remaining 5 widgets (Sign, OCR, Edit, Compress, Convert) still live only in the prototype. See `docs/phase-6-tool-widget-port-plan.md`.
- **Next workstream** — Phase 6 Step 4 (port Sign — largest widget by line count, exercises method tabs + draw/type/upload modes + draggable signature field). Or branch out to extract `I18N_ARTICLES`. See `docs/phase-6-tool-widget-port-plan.md` §3 for the full port order.
