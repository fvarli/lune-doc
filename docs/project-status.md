...
- **Phase 6 (port real tool widgets into the workspace)** — IN PROGRESS, started 2026-05-03. New package `@lunedoc/tools` alongside `@lunedoc/ui` and `@lunedoc/i18n`. **Two widgets ported so far:** Merge PDF at `/merge-pdf` (commit `450fad8`) and Split PDF at `/split-pdf` (commit `876aee7`). `apps/web` uses `react-router-dom` v7 with a shared `<Header>` / `<Footer>` layout and lang state shared across routes — switching locale on either tool route updates Header, tool content, and Footer simultaneously. The remaining 6 widgets (Watermark, Sign, OCR, Edit, Compress, Convert) still live only in the prototype. See `docs/phase-6-tool-widget-port-plan.md`.
- **Next workstream** — Phase 6 Step 3 (port Watermark — first widget with controls + preview pane layout). Or branch out to extract `I18N_ARTICLES`. See `docs/phase-6-tool-widget-port-plan.md` §3 for the full port order.
