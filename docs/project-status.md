...
- **Frontend migration**: ✓ DONE through Phase 7 on `main`, then **consolidated** — `apps/web` (Vite + React 19 + TS, port 5173) is now the **only** frontend app. `apps/marketing` was deleted on 2026-05-05 (D10 follow-through). The 25 SEO landing routes (1 home + 8 tools × 3 locales + 2 localized homes = 27 generated) now ship from `apps/web` via a small `tsx scripts/prerender.ts` post-build step that writes per-route static HTML with correct `<title>`, `<meta>`, canonical, hreflang × 4, and 4 JSON-LD blocks (SoftwareApplication / FAQPage / HowTo / BreadcrumbList) — same SEO surface as the old Astro output. The body remains SPA-rendered after hydration; Google sees full content, JS-disabled crawlers see static head metadata. **Merge + Split + Watermark are wired to the real backend** via `@lunedoc/api`; the remaining 5 tools still run client-side mocks. See §8 R3 for commit ranges.
- **Backend**: Phase 0 done; **Phase 1 in progress** — Merge + Split + **Watermark** live both server-side and frontend-wired, Sign next. `services/api/` (FastAPI + async SQLAlchemy + asyncpg + Celery + Redis + LocalDiskStorage + PyMuPDF) ships the anonymous file lifecycle (POST/GET/DELETE/download under `/api/v1/files`, with optional `X-Owner-Token` to extend ownership across multiple uploads), `/api/v1/healthz`, owner-token HMAC auth, MIME whitelist (415), 50 MB cap (413), 60 s TTL sweeper, and a `Job` model (with `params` JSONB) backing `POST /api/v1/jobs/{merge,split,watermark}`, `GET /api/v1/jobs/{id}`, `GET /api/v1/jobs/{id}/result` (multi-output aware). Other tool endpoints still stubbed at 501. See `docs/backend-api-plan.md` and `services/api/README.md`. Postgres + Redis run as host services; **no Docker**.
...
- **Phase 1: Merge / Split / Watermark / Sign / Edit — IN PROGRESS** (Merge + Split + **Watermark** server-side **and** frontend-wired as of 2026-05-05). Job model + `jobs` table (Alembic 0002 + 0003 added a `params` JSONB column for per-tool config); PyMuPDF as the engine; Celery `lunedoc.merge`, `lunedoc.split`, and `lunedoc.watermark` tasks share the `queued → running → done|failed` lifecycle. Route surface: `POST /api/v1/jobs/merge`, `POST /api/v1/jobs/split` (mode=`ranges` with 1-indexed inclusive `[start,end]` lists, or mode=`per_page`), `POST /api/v1/jobs/watermark` (text + position ∈ {center, top-left, top-right, bottom-left, bottom-right} + opacity 0.1–1 + rotation -180–180), shared `GET /api/v1/jobs/{id}` and `GET /api/v1/jobs/{id}/result` (multi-output aware). Watermark uses `pymupdf.TextWriter` + matrix to support arbitrary rotation angles (PyMuPDF's `Page.insert_text(rotate=…)` only accepts multiples of 90). Outputs are normal `File` rows inheriting the job's owner_token_hash, downloadable via `/api/v1/files/{id}/download`. Same X-Owner-Token no-leak policy as files. **`POST /api/v1/files` accepts an optional `X-Owner-Token` header to extend ownership across multiple uploads — needed for multi-input flows like Merge.** **31/31 pytest green** (was 25, +6 watermark). Frontend client (`@lunedoc/api`) covers `createMergeJob`/`createSplitJob`/`createWatermarkJob` and the full upload → poll → result → download lifecycle. Tools remaining in this phase: Sign, Edit.
