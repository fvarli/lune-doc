...
- **Phase 7 — pick a workstream:**
  - **Option A (recommended): scaffold `apps/marketing` with Astro** + the 8 SEO tool landing pages per `docs/seo-tool-page-template.md`. The architecture is already documented; it consumes `@lunedoc/ui`, `@lunedoc/i18n`, and `@lunedoc/tools` as React islands. The 8 widgets just ported are the exact pieces marketing pages hydrate. **Why first:** zero new design work, all infra and content templates exist, and SEO pages are the actual product surface that users find via Google. About 1–2 sessions of work.
  - **Option B: open backend MVP** per `docs/backend-api-plan.md` (FastAPI + Celery + Redis, Phase 0 skeleton → tools → auth). Real product capability behind the widgets. Larger workstream (~7 weeks per the plan), and the widgets stay client-side mocks until the API endpoints are reachable.
  - **Option C: extract `I18N_ARTICLES`** from the prototype into `@lunedoc/i18n` (small, ~30 minutes). Closes the open thread from Phase 4. Worth doing whenever; not blocking.
  - **My preferred next step: Option A.** Builds on what just shipped, exercises the package boundaries the migration was designed for, and produces something real users can land on.
