...
**Snapshot date:** 2026-05-09 (Phase 4 Step 3A — backend quota in progress)
...
- **Backend**: Phase 0 done; Phase 1 done; Phase 2 done; Phase 3 done; **Phase 4 Steps 1, 2A, 2B, 2C ✓ COMPLETE; Step 3A IN PROGRESS** as of 2026-05-09 — backend auth foundation (email passwordless, JWT access + DB refresh, claim, `can_access` boundary), the three authenticated `/me/*` dashboard endpoints (`/me/jobs`, `/me/files`, `/me/usage`), the frontend auth shell (`AuthContext` + `/signin` + `/auth/verify` + auth-aware Header), and the authenticated frontend dashboard page (`/dashboard`, `/tr/dashboard`, `/es/dashboard`) are all live. `apps/web` now ships a localized `DashboardPage` that loads usage / recent jobs / recent files in parallel via `withAccessToken`, redirects unauthenticated visitors to localized `/signin`, and shows a Dashboard link in the header when signed in. `@lunedoc/api` exports `getMeJobs / getMeFiles / getMeUsage` plus the matching `Me*` DTOs; `owner_token_hash` is structurally absent. **Step 3A** (backend quota enforcement: 20 OCR pages/day, 60 jobs/hour, 3 concurrent jobs; Redis-backed; 429 with structured payload) is in progress on `main`. Remaining Phase 4 work: Step 3B (frontend quota UX: banners, dashboard usage strip, upgrade CTA), paid tiers, and optional Google OAuth. `services/api/` (FastAPI + async SQLAlchemy + asyncpg + Celery + Redis + LocalDiskStorage + PyMuPDF + **Ghostscript** + **headless LibreOffice** + **Tesseract** subprocesses) ships the anonymous file lifecycle (POST/GET/DELETE/download under `/api/v1/files`, with optional `X-Owner-Token` to extend ownership across multiple uploads), `/api/v1/healthz`, owner-token HMAC auth, MIME whitelist (415), 50 MB cap (413), 60 s TTL sweeper, and a `Job` model (with `params` JSONB) backing `POST /api/v1/jobs/{merge,split,watermark,sign,edit,compress,convert,ocr}`, `GET /api/v1/jobs/{id}`, `GET /api/v1/jobs/{id}/result` (multi-output aware). All eight job endpoints live; nothing left at 501. See `docs/backend-api-plan.md` and `services/api/README.md`. Postgres + Redis + Ghostscript + LibreOffice + Tesseract (with `eng/tur/spa` packs) run as host services; **no Docker**.
