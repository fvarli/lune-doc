...
- **Frontend migration**: ✓ DONE through Phase 7 on `main`, then **consolidated** — `apps/web` (Vite + React 19 + TS, port 5173) is now the **only** frontend app. `apps/marketing` was deleted on 2026-05-05 (D10 follow-through). The 25 SEO landing routes (1 home + 8 tools × 3 locales + 2 localized homes = 27 generated) now ship from `apps/web` via a small `tsx scripts/prerender.ts` post-build step that writes per-route static HTML with correct `<title>`, `<meta>`, canonical, hreflang × 4, and 4 JSON-LD blocks (SoftwareApplication / FAQPage / HowTo / BreadcrumbList) — same SEO surface as the old Astro output. The body remains SPA-rendered after hydration; Google sees full content, JS-disabled crawlers see static head metadata. **All 5 Phase 1 tools (Merge / Split / Watermark / Sign / Edit) are wired to the real backend** via `@lunedoc/api`; the remaining 3 tools (Compress / Convert / OCR) still run client-side mocks pending Phase 2 + 3. See §8 R3 for commit ranges.
- **Backend**: Phase 0 done; **Phase 1 ✓ COMPLETE** as of 2026-05-05 — Merge + Split + Watermark + Sign + **Edit** all live both server-side and frontend-wired. `services/api/` (FastAPI + async SQLAlchemy + asyncpg + Celery + Redis + LocalDiskStorage + PyMuPDF) ships the anonymous file lifecycle (POST/GET/DELETE/download under `/api/v1/files`, with optional `X-Owner-Token` to extend ownership across multiple uploads), `/api/v1/healthz`, owner-token HMAC auth, MIME whitelist (415), 50 MB cap (413), 60 s TTL sweeper, and a `Job` model (with `params` JSONB) backing `POST /api/v1/jobs/{merge,split,watermark,sign,edit}`, `GET /api/v1/jobs/{id}`, `GET /api/v1/jobs/{id}/result` (multi-output aware). Phase 2 endpoints (`/jobs/{compress,convert}`) and Phase 3 (`/jobs/ocr`) still stubbed at 501. See `docs/backend-api-plan.md` and `services/api/README.md`. Postgres + Redis run as host services; **no Docker**.
