...
- **Phase 7 (`apps/marketing` Astro site for SEO tool landing pages)** — IN PROGRESS, started 2026-05-03 with the Astro scaffold (commit `749e685`). `apps/marketing` is a 4th workspace alongside `apps/web` and the three packages, runs on port **4321** (no collision with prototype's 8765 or web's 5173), consumes `@lunedoc/ui` / `@lunedoc/i18n` / `@lunedoc/tools` as workspace deps. **OCR PDF landing page live at `/ocr-pdf`, `/tr/ocr-pdf`, `/es/ocr-pdf`** (commit `0659985`). Each page emits 4 JSON-LD blocks (SoftwareApplication, FAQPage, HowTo, BreadcrumbList), full hreflang set (en/tr/es/x-default), per-locale `<title>` and `<html lang>`, and hydrates the `OCRToolPage` widget as a React island. **1 of 8 tool pages done.** Shared `ToolLandingLayout.astro` + `seo/schema.ts` + `MarketingHeader.tsx` patterns established for the remaining 7. See `docs/phase-7-marketing-scaffold-plan.md`.
- **Next workstream — port the next tool landing page.** Recommended: **Watermark PDF** (smallest payload after OCR; validates the layout's sticky-preview pattern). Then Sign / Edit (richer), then Compress / Convert (state machine + format pair — different shape; useful as the layout's stress test). Each port is now ~3 files (data + EN page + TR/ES page); the layout and schema helpers are reused as-is.
