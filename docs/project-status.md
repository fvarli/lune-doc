...
- **Backend**: Phase 0 done; Phase 1 done; Phase 2 done; Phase 3 done; **Phase 4 Step 1 + Step 2A ✓ COMPLETE** as of 2026-05-08 — backend auth foundation (email passwordless, JWT access + DB refresh, claim, `can_access` boundary) and the three authenticated `/me/*` dashboard endpoints (`/me/jobs`, `/me/files`, `/me/usage`) are live; **frontend auth shell (Step 2B) is in progress** — `@lunedoc/api` has the auth client methods, `apps/web` ships an `AuthContext` + `/signin` (email + 6-digit code) + `/auth/verify` magic-link route + auth-aware Header (Sign in/Get started → email + Logout); **dashboard page consuming `/me/*` is still pending Step 2C**. `services/api/` (FastAPI + async SQLAlchemy + asyncpg + Celery + Redis + LocalDiskStorage + PyMuPDF + **Ghostscript** + **headless LibreOffice** + **Tesseract** subprocesses) ships the anonymous file lifecycle (POST/GET/DELETE/download under `/api/v1/files`, with optional `X-Owner-Token` to extend ownership across multiple uploads), `/api/v1/healthz`, owner-token HMAC auth, MIME whitelist (415), 50 MB cap (413), 60 s TTL sweeper, and a `Job` model (with `params` JSONB) backing `POST /api/v1/jobs/{merge,split,watermark,sign,edit,compress,convert,ocr}`, `GET /api/v1/jobs/{id}`, `GET /api/v1/jobs/{id}/result` (multi-output aware). All eight job endpoints live; nothing left at 501. See `docs/backend-api-plan.md` and `services/api/README.md`. Postgres + Redis + Ghostscript + LibreOffice + Tesseract (with `eng/tur/spa` packs) run as host services; **no Docker**.
...
- **Phase 4 Step 2B: Frontend auth shell — ⏳ IN PROGRESS** (2026-05-08). `@lunedoc/api` wired to `/auth/email/start`, `/auth/email/verify`, `/auth/refresh`, `/auth/logout`, `/auth/me`, `/auth/claim` (new `UnauthorizedError` for 401, new `auth_storage` module for refresh-token persistence, new `listAllOwnerTokens` helper). `apps/web` adds `AuthProvider` (mounts inside `BrowserRouter`; refresh on boot guarded against StrictMode double-invoke; `withAccessToken` retry-once helper defined for Step 2C), a two-stage `/signin` page (email → 6-digit code with resend + use-different-email + dev-mode "check terminal" hint), and `/auth/verify` magic-link consumer. After a successful verify, all known anonymous `owner_token`s are POSTed to `/auth/claim` (silent on failure). Header gains an optional `rightSlot` so apps wire auth-aware controls without `packages/ui` taking a `react-router-dom` dep — signed out shows Sign in + Get started linking to `/signin`; signed in shows the user email + Logout. 14 new i18n keys in EN/TR/ES. **Dashboard page consuming `/me/jobs|files|usage` is still pending Step 2C.**
- Phase 4 Step 2C: Frontend dashboard page consuming `/me/*` (next).
