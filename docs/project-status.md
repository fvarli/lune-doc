...
- **Frontend migration**: ✓ DONE through Phase 7 on `main`, then **consolidated** — `apps/web` (Vite + React 19 + TS, port 5173) is now the **only** frontend app. `apps/marketing` was deleted on 2026-05-05 (D10 follow-through). The 25 SEO landing routes (1 home + 8 tools × 3 locales + 2 localized homes = 27 generated) now ship from `apps/web` via a small `tsx scripts/prerender.ts` post-build step that writes per-route static HTML with correct `<title>`, `<meta>`, canonical, hreflang × 4, and 4 JSON-LD blocks (SoftwareApplication / FAQPage / HowTo / BreadcrumbList) — same SEO surface as the old Astro output. The body remains SPA-rendered after hydration; Google sees full content, JS-disabled crawlers see static head metadata. **6 of 8 tools are wired to the real backend** (Merge / Split / Watermark / Sign / Edit / Compress); the remaining 2 (Convert / OCR) still run client-side mocks pending Phase 2 part 2 + Phase 3. See §8 R3 for commit ranges.
- **Backend**: Phase 0 done; Phase 1 ✓ done; **Phase 2 in progress** — Compress live (server-side + frontend-wired) as of 2026-05-05. `services/api/` (FastAPI + async SQLAlchemy + asyncpg + Celery + Redis + LocalDiskStorage + PyMuPDF + **Ghostscript subprocess**) ships the anonymous file lifecycle (POST/GET/DELETE/download under `/api/v1/files`, with optional `X-Owner-Token` to extend ownership across multiple uploads), `/api/v1/healthz`, owner-token HMAC auth, MIME whitelist (415), 50 MB cap (413), 60 s TTL sweeper, and a `Job` model (with `params` JSONB) backing `POST /api/v1/jobs/{merge,split,watermark,sign,edit,compress}`, `GET /api/v1/jobs/{id}`, `GET /api/v1/jobs/{id}/result` (multi-output aware). Phase 2 part 2 endpoint `/jobs/convert` and Phase 3 `/jobs/ocr` still stubbed at 501. See `docs/backend-api-plan.md` and `services/api/README.md`. Postgres + Redis run as host services; **no Docker**.
...
- **Phase 2: Compress + Convert — IN PROGRESS** (Compress live as of 2026-05-05; Convert next). `lunedoc.compress` Celery task wraps a `compress_pdf` engine that prefers **Ghostscript** (`gs`, host binary at `/usr/bin/gs 10.02.1`) for real compression (30–80% reduction on image-heavy PDFs via `-dPDFSETTINGS=/screen|/ebook|/printer`) and falls back to PyMuPDF's deflate-based save (modest 0–15%) when `gs` isn't on PATH. The engine guarantees output_bytes ≤ input_bytes — if the chosen path produces a worse file, the original bytes are shipped through. Route: `POST /api/v1/jobs/compress` with `{file_id, level: low|medium|high}`. Job result_meta records the engine used + byte counts, so the frontend renders "Saved X MB (Y%)". `createCompressJob` shipped in `@lunedoc/api`. **57/57 pytest green** (was 48, +9 compress including parametrized levels + Ghostscript path + PyMuPDF fallback via monkeypatch + no-regression guarantee). Phase 2 part 2 (Convert: PDF↔JPG/PNG via PyMuPDF + DOCX/XLSX/PPTX↔PDF via headless LibreOffice) is the next workstream.
