...
- **Phase 7 (`apps/marketing` Astro site for SEO tool landing pages)** — IN PROGRESS, started 2026-05-03 with the Astro scaffold (commit `749e685`). `apps/marketing` is a 4th workspace alongside `apps/web` and the three packages, runs on port **4321** (no collision with prototype's 8765 or web's 5173), consumes `@lunedoc/ui` / `@lunedoc/i18n` / `@lunedoc/tools` as workspace deps, and renders a placeholder home page with a real `<Logo>` React island. Build produces static HTML with the Logo SVG fully SSR'd plus a `<astro-island client="load">` wrapper for hydration. **No tool landing pages ported yet.** See `docs/phase-7-marketing-scaffold-plan.md`.
- **Next workstream — port the first tool landing page** following `docs/seo-tool-page-template.md`. **Recommended: OCR PDF** (`/ocr-pdf` and `/tr/ocr-pdf`, `/es/ocr-pdf`) since the template's filled example is OCR — strongest forcing function. Watermark or Merge are simpler alternatives if you want to validate the template shape with smaller copy before tackling OCR. Once one tool page is shippable, the remaining 7 follow with smaller diffs.
